...
func (web *Web) liteAudienceDisplayHandler(w http.ResponseWriter, r *http.Request) {
	if !web.enforceDisplayConfiguration(w, r, map[string]string{"background": "#0f0", "reversed": "false",
		"overlayLocation": "bottom"}) {
		return
	}

	template, err := web.parseFiles("templates/lite_audience_display.html")
	if err != nil {
		handleWebErr(w, err)
		return
	}
	...
	err = template.ExecuteTemplate(w, "lite_audience_display.html", data)
	if err != nil {
		handleWebErr(w, err)
		return
	}
...
func (web *Web) liteAudienceDisplayWebsocketHandler(w http.ResponseWriter, r *http.Request) {
