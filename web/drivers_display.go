...
func (web *Web) driversDisplayHandler(w http.ResponseWriter, r *http.Request) {
	if !web.enforceDisplayConfiguration(w, r, map[string]string{"background": "#0f0", "reversed": "false",
		"overlayLocation": "bottom"}) {
		return
	}

	template, err := web.parseFiles("templates/drivers_display.html")
	if err != nil {
		handleWebErr(w, err)
		return
	}
	...
	err = template.ExecuteTemplate(w, "drivers_display.html", data)
	if err != nil {
		handleWebErr(w, err)
		return
	}
...
func (web *Web) driversDisplayWebsocketHandler(w http.ResponseWriter, r *http.Request) {
